...
function saveDatabase() {
    writeYaml('.database.yml', database, function(err) {
...
function updateComment(url, request) {
    ...
    const updatedComment = request && request.body && request.body.comment;
    const response = {};
...
function deleteComment(url, request) {
    const commentId = url && url.split('/')[2];
    const comment = database.comments[commentId];
    const response = {};

    if(comment) {
        const articleCommentArray = database.articles[comment.articleId].commentIds;
        const usersCommentArray = database.users[comment.username].commentIds;
        articleCommentArray.splice(articleCommentArray.indexOf(comment.id), 1);
        usersCommentArray.splice(usersCommentArray.indexOf(comment.id), 1);
...
function upOrDownVoteComment(upvote, url, request) {
    const commentId = url && url.split('/')[2];
    const comment = database.comments[commentId];
    const username = request.body && request.body.username;
    const response = {};

    if(!comment || !commentId || !database.users[username]) {
        response.status = 400;
    }
    else {
        const removeFrom = (upvote) ? comment.downvotedBy : comment.upvotedBy;
        const addTo = (upvote) ? comment.upvotedBy : comment.downvotedBy;
        if(addTo.indexOf(username) === -1) {
            addTo.push(username);

            const index = removeFrom.indexOf(username);
            if(index > -1) {
...
function upvoteComment(url, request) {
    return upOrDownVoteComment(true, url, request);
}

function downvoteComment(url, request) {
    return upOrDownVoteComment(false, url, request);
}
